...
%     % Dentro/fuera
%     % sr
%     [Kin_sr,Kon_sr] = inpolygon(phi,Kv,K_sr(:,1),K_sr(:,2)*escala(i).*sqrt(K_sr(:,1)));
%     nK_sr(i)=numel(phi(Kin_sr))+numel(phi(Kon_sr));
%     [Gin_sr,Gon_sr] = inpolygon(phi,Gv,G_sr(:,1),G_sr(:,2)*escala(i).*sqrt(G_sr(:,1)));
%     nG_sr(i)=numel(phi(Gin_sr))+numel(phi(Gon_sr));
%     nKG_sr(i)=nK_sr(i)+nG_sr(i);
% 
%     % pol
%     [Kin_pol,Kon_pol] = inpolygon(phi,Kv,K_pol(:,1),K_pol(:,2)*escala(i).*sqrt(K_pol(:,1)));
%     nK_pol(i)=numel(phi(Kin_pol))+numel(phi(Kon_pol));
%     [Gin_pol,Gon_pol] = inpolygon(phi,Gv,G_pol(:,1),G_pol(:,2)*escala(i).*sqrt(G_pol(:,1)));
...
% %     % Graficos
% %     subplot(2,1,1);
% %     plot(K_sr(:,1),K_sr(:,2)*escala(i).*sqrt(K_sr(:,1)));
% %     plot(K_pol(:,1),K_pol(:,2)*escala(i).*sqrt(K_pol(:,1)));
% %     xlim([0 0.45]);
% %     ylim([0 0.2]);
% %     hold on
...
% %     
% %     subplot(2,1,2);
% %     plot(G_sr(:,1),G_sr(:,2)*escala(i).*sqrt(G_sr(:,1)));    
% %     xlim([0 0.45]);
% %     ylim([0 0.3]);
% %     hold on
...
% % Dentro/fuera
% % % sr
% % [Kin_sr,Kon_sr] = inpolygon(phi,Kv,K_sr(:,1),K_sr(:,2)*esc.*sqrt(K_sr(:,1)));
% % nK_sr(i)=numel(phi(Kin_sr))+numel(phi(Kon_sr));
% % [Gin_sr,Gon_sr] = inpolygon(phi,Gv,G_sr(:,1),G_sr(:,2)*esc.*sqrt(G_sr(:,1)));
% % nG_sr(i)=numel(phi(Gin_sr))+numel(phi(Gon_sr));
% % nKG_sr(i)=nK_sr(i)+nG_sr(i);
% 
% % pol
% [Kin_pol,Kon_pol] = inpolygon(phi,Kv,K_pol(:,1),K_pol(:,2)*esc.*sqrt(K_pol(:,1)));
% nK_pol(i)=numel(phi(Kin_pol))+numel(phi(Kon_pol));
% [Gin_pol,Gon_pol] = inpolygon(phi,Gv,G_pol(:,1),G_pol(:,2)*esc.*sqrt(G_pol(:,1)));
% nG_pol(i)=numel(phi(Gin_pol))+numel(phi(Gon_pol));
% nKG_pol(i)=nK_pol(i)+nG_pol(i);
% 
% 
...
% % 
% % subplot(2,1,2);
% % plot(G_sr(:,1),G_sr(:,2)*esc.*sqrt(G_sr(:,1)));
% % xlim([0 0.45]);
% % ylim([0 0.3]);
% % hold on
% % plot(phi(Gin_sr),Gv(Gin_sr),'r+')  % puntos adentro
...
% figure('Position',[400 200 400 800]);
% subplot(2,1,1);
% plot(K_pol(:,1),K_pol(:,2)*esc.*sqrt(K_pol(:,1)));
% xlim([0 0.45]);
% ylim([0 0.2]);
% hold on
% plot(phi(Kin_pol),Kv(Kin_pol),'r+')  % puntos adentro
...
% 
% subplot(2,1,2);
% plot(G_pol(:,1),G_pol(:,2)*esc.*sqrt(G_pol(:,1)));
% xlim([0 0.45]);
% ylim([0 0.3]);
% hold on
% plot(phi(Gin_pol),Gv(Gin_pol),'r+')  % puntos adentro
...
K_pol=vertcat(maxK_pol,flip(minK_pol));
K_tc=vertcat(maxK_tc,flip(minK_tc));
K_sr(:,2)=K_sr(:,2)*esc.*sqrt(K_sr(:,1));
K_pol(:,2)=K_pol(:,2)*esc.*sqrt(K_pol(:,1));
K_tc(:,2)=K_tc(:,2)*esc.*sqrt(K_tc(:,1));

G_sr=vertcat(maxG_sr,flip(minG_sr));
G_pol=vertcat(maxG_pol,flip(minG_pol));
G_tc=vertcat(maxG_tc,flip(minG_tc));
G_sr(:,2)=G_sr(:,2)*esc.*sqrt(G_sr(:,1));
G_pol(:,2)=G_pol(:,2)*esc.*sqrt(G_pol(:,1));
G_tc(:,2)=G_tc(:,2)*esc.*sqrt(G_tc(:,1));
