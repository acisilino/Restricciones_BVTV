...
function [hK_VR,hG_VR,hK_G,hG_G,hK_HS,hG_HS]=KG_Specimens_HS
...
for i=1: ntensoresVR
    KvVR(i)= (1/9)*(CVR(i,1,1)+CVR(i,2,2)+CVR(i,3,3)+2*(CVR(i,1,2)+...
        CVR(i,2,3)+CVR(i,3,1)));
    GvVR(i)=(1/15)*(CVR(i,1,1)+CVR(i,2,2)+CVR(i,3,3)-(CVR(i,1,2)+...
        CVR(i,2,3)+CVR(i,3,1))+3*(CVR(i,4,4)+CVR(i,5,5)+CVR(i,6,6)));
end
...
for i=1: ntensoresG
    KvG(i)= (1/9)*(CG(i,1,1)+CG(i,2,2)+CG(i,3,3)+2*(CG(i,1,2)+...
        CG(i,2,3)+CG(i,3,1)));
    GvG(i)=(1/15)*(CG(i,1,1)+CG(i,2,2)+CG(i,3,3)-(CG(i,1,2)+...
        CG(i,2,3)+CG(i,3,1))+3*(CG(i,4,4)+CG(i,5,5)+CG(i,6,6)));
end
...
subplot(2,1,1);
hold on;
% hK_VR=scatter(phiVR(:),KvVR(:),'ok','MarkerFaceColor','k','DisplayName',...
%    'Human specimens [ref]'); 
% hK_G=scatter(phiG(:),KvG(:),'oK','DisplayName','Bovine specimens [ref]'); 
hK_VR=0;
hK_G=0;
...
subplot(2,1,2);
hold on;
% hG_VR=scatter(phiVR(:),GvVR(:),'ok','MarkerFaceColor','k','DisplayName',...
%    'Human specimens [ref]'); 
% hG_G=scatter(phiG(:),GvG(:),'oK','DisplayName','Bovine specimens [ref]'); 
hG_VR=0;
hG_G=0;
